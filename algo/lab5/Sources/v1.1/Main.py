...
def main():
    ...
    for mutator in Config.MUTATORS:
        best_values = MakeExperiment(
            ...
            mutator = Config.MUTATORS[mutator],
            crossover = Config.CROSSOVERS['Half'],
            local_optimizer = Config.LOCAL_OPTIMIZERS['AddLightest']
        )

        results_of_mutators[mutator] = best_values

    best_mutation = max(
        [
            {'var': k, 'max': results_of_mutators[k][-1]}
            for k in results_of_mutators
        ],
        key = lambda x: x['max']
    )['var']
    ...
    PrintBestStat('mutator', results_of_mutators)
    print()
