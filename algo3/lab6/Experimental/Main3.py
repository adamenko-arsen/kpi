...
class YahtzeeGUI:
    ...
    def minimax(self, scores, available_categories, depth, is_maximizing, alpha, beta):
        if depth == 0 or not available_categories:
        ...
        if is_maximizing:
            ...
            for category in available_categories:
                ...
                new_scores[category] = self.calculate_score(category)
                eval_score = self.minimax(
                    new_scores, [c for c in available_categories if c != category], depth - 1, False, alpha, beta
                )
            ...
            for category in available_categories:
                ...
                new_scores[category] = self.calculate_score(category)
                eval_score = self.minimax(
                    new_scores, [c for c in available_categories if c != category], depth - 1, True, alpha, beta
                )
    ...
    def bot_turn(self):
        ...
        best_category = None
        for category in available_categories:
            temp_scores = self.bot_scores.copy()
            temp_scores[category] = self.calculate_score(category)
            score = self.minimax(temp_scores, [c for c in available_categories if c != category], 1, False, float("-inf"), float("inf"))
            if score > best_score:
                best_score = score
