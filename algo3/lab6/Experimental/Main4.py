...
class YahtzeeGUI:
    ...
    def create_widgets(self):
        ...
        self.bot_move_label.grid(row=3, column=0, columnspan=5)

        self.bot_score_label = tk.Label(self.root, text="Bot Score: 0", font=("Arial", 14))
        self.bot_score_label.grid(row=4, column=0, columnspan=2)

        self.player_score_label = tk.Label(self.root, text="Your Score: 0", font=("Arial", 14))
        self.player_score_label.grid(row=4, column=3, columnspan=2)
    ...
    def minimax_decision(self, available_categories):
        ...
        for category in available_categories:
            score = self.minimax(category, depth=8, alpha=float('-inf'), beta=float('inf'), maximizing_player=True)
            if score > best_score:
                best_score = score
